...
public class ClientGlobal {
     ...
     *
     */
    public static void init(FastdfsProperties properties) throws IOException, MyException {
        ...
        g_connect_timeout = properties.getConnectTimeout(); //iniReader.getIntValue(CONF_KEY_CONNECT_TIMEOUT, DEFAULT_CONNECT_TIMEOUT);
        ...
        g_network_timeout = properties.getNetworkTimeout(); //iniReader.getIntValue(CONF_KEY_NETWORK_TIMEOUT, DEFAULT_NETWORK_TIMEOUT);
        ...
        g_charset = properties.getCharset(); //iniReader.getStrValue(CONF_KEY_CHARSET);
        if (g_charset == null || g_charset.length() == 0) {
            g_charset = "ISO8859-1";
        }

        szTrackerServers = Tools.splitString(properties.getTrackerServers(), ","); // iniReader.getValues(CONF_KEY_TRACKER_SERVER);
        ...
        g_tracker_http_port = properties.getTrackerHttpPort();// iniReader.getIntValue(CONF_KEY_HTTP_TRACKER_HTTP_PORT, 80);
        g_anti_steal_token = properties.isAntiStealToken();//iniReader.getBoolValue(CONF_KEY_HTTP_ANTI_STEAL_TOKEN, false);
        if (g_anti_steal_token) {
            g_secret_key = properties.getSecretKey(); //iniReader.getStrValue(CONF_KEY_HTTP_SECRET_KEY);
        }
    }
